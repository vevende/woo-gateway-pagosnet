...
class PagosNetGateway extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->icon               = plugins_url( 'images/woocommerce-pagosnet.png', plugin_dir_path( __FILE__ ) );
		$this->method_title       = 'PagosNet';
		$this->method_description = 'Pago en linea via PagosNet Bolivia';
		$this->order_button_text  = 'Pagar con PagosNet';
		...
		if ( ! $this->is_valid_for_use() ) {
			...
			add_action( 'woocommerce_update_options_payment_gateways_pagosnet', array(
				$this,
				'process_admin_options'
			) );
			add_action( 'woocommerce_api_wc_gateway_pagosnet', array( $this, 'return_handler' ) );
	...
	public function admin_options() {
		if ( $this->is_valid_for_use() ) {
            ...
            <div class="inline error"><p>
                    <strong><?php _e( 'Gateway disabled', 'woocommerce' ); ?></strong>: <?php _e( 'PagosNet does not support your store currency.', 'woocommerce' ); ?>
                </p></div>
			<?php
		}
	}
	...
	public function receipt_page( $order_id ) {
		echo '<p>Muchas Gracias, por favor paga con PagosNet</p>';
		$order  = wc_get_order( $order_id );
		$posted = wp_unslash( $_POST );

		if ( isset( $posted['pagosnet-phonenumber'] ) ) {
			$phone = trim( $posted['pagosnet-phonenumber'] );

			if ( preg_match( "/^[6-7][0-9]{7}$/", $phone ) ) {
				$pagosnet = new WC_Gateway_PagosNet_Request( $this );
				$pay_url   = $pagosnet->get_request_url( $order, $posted['pagosnet-phonenumber'] );
				$mensaje   = $pagosnet->generate_arguments( $order, $posted['pagosnet-phonenumber'] );
				$pagosnet->pagoPagosNet( $mensaje, $order );
				exit();
			} else {
	...
	public function generate_form( $order ) {
		...
		$form .= '<input type="text" id="id_pagosnet-phonenumber" name="pagosnet-phonenumber" required= value="" /> ';
		$form .= '<br><br>';
		$form .= '<input type="submit" class="button alt" id="submit-payment-form" value="Pagar con pagosnet" /> ';
	...
	function return_handler() {
		...
		if ( ! empty( $_GET ) ) {
			if ( array_key_exists( 'r', $_GET ) ) {
				$pagosnet = new WC_Gateway_pagosnet_Request( $this );
				$posted    = $pagosnet->get_response_arguments( str_replace( ' ', '+', $_GET['r'] ) );
				...
				wp_die( 'Request Failure', 'pagosnet Request', array( 'response' => 200 ) );
				exit();
			}
		}
	...
	protected function payment_status_failed( $order, $posted ) {
		$errormessages = array(
			...
			'1001' => 'Los fondos en su Billetera movil son insuficientes, para cargar su billetera vaya al Punto PagosNet mas cercano, marque *555#',
		...
		if ( array_key_exists( 'codRes', $posted ) ) {
			wc_add_notice( 'PagosNet > ' . $errormessages[ $posted['codRes'] ], 'error' );
		} else {
			wc_add_notice( 'PagosNet > Lo sentimos, hubo un error desconocido', 'error' );
		}
	...
	protected function payment_status_completed( $order, $posted ) {
		...
		wc_add_notice( 'PagosNet > ' . $posted['mensaje'], 'success' );
